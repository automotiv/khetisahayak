DECLARE
   -- Define category codes to process (you can modify this list)
   TYPE category_array IS TABLE OF VARCHAR2(50);
   v_category_codes category_array := category_array('MP0001', 'MP0002', 'MP0003');
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE('=== PRODUCT ANALYSIS SCRIPT WITH CATEGORY LIST STARTED ===');
   ...
   DBMS_OUTPUT.PUT_LINE('Debug: Number of categories to process: ' || v_category_codes.COUNT);
   DBMS_OUTPUT.PUT_LINE('Debug: Category codes: ' || v_category_codes(1) || ', ' || v_category_codes(2) || ', ' || v_category_codes(3));
   DBMS_OUTPUT.PUT_LINE('===========================================');

   -- Process each category code from the array
   FOR i IN 1..v_category_codes.COUNT LOOP
      v_category_code := v_category_codes(i);
      v_category_count := v_category_count + 1;
      
      DBMS_OUTPUT.PUT_LINE('');
      DBMS_OUTPUT.PUT_LINE('Debug: Processing category ' || v_category_count || ' of ' || v_category_codes.COUNT || ': ' || v_category_code);
      ...
      DBMS_OUTPUT.PUT_LINE('Debug: Starting product processing loop for category: ' || v_category_code);
      FOR prod_rec IN (
         ...
         DBMS_OUTPUT.PUT_LINE('');
         DBMS_OUTPUT.PUT_LINE('Debug: Processing product ' || v_processed_count || ' (Category: ' || v_category_code || ')');
      ...
      DBMS_OUTPUT.PUT_LINE('Debug: Completed processing for category: ' || v_category_code);
   END LOOP;
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      DBMS_OUTPUT.PUT_LINE('Debug: Categories processed before error: ' || v_category_count);
END;
/ 